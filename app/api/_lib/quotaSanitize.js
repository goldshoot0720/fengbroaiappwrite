 ...
 * 最後才當 ChatGPT（任何其他 JWT／session.json）。
 */
...
/**
 * accessToken 是可直接呼叫 ChatGPT / Claude / Grok API 的憑證，額度 API 一律不回傳明文；
...
export function sanitizeQuotaRow(row) {
  ...
  return {
    ...
    accessTokenHint:
      buildAccessTokenHint(accessToken) || buildClaudeAccessTokenHint(accessToken) || buildGrokAccessTokenHint(accessToken),
