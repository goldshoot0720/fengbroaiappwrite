...
> **最後更新**: 2026-09-03  
> **現況**: 選單與 17 張表以 [INDEX.md](./INDEX.md) 為準。資料表欄位以 `app/api/create-table/route.js` 的 `TABLE_SCHEMAS` 為準。
...
| 訂閱管理 | 管理各種訂閱服務與付費週期 |
| 試用／首購 | 依服務展開多帳號，標示試用與首購狀態 |
| 重灌軟體 | 整理 Windows／Mac 重灌清單，序號預設隱藏 |
| 筆記管理 | 撰寫、整理個人筆記與文章 |
| 常用帳號 | 收藏常用網站連結與備忘 |
| 圖片管理 | 瀏覽與管理圖片庫 |
| 影片管理 | 管理與播放影片收藏 |
...
鋒兄首頁進入系統後的預設畫面，首頁頂端可在「精簡」與「完整儀表」兩種檢視間切換。
...
表單另外可填分類、用途、頻率、友善度、替代方案、去留建議與封存。封存後預設不出現在列表，可從「已封存」找回。儀表到期、月費與推播／信件仍計算全部訂閱。既有 8 欄資料表請到「鋒兄設定」按 **補欄位**。
...
入口：**鋒兄管理 → 試用/首購**。一筆紀錄代表「一個服務的一個帳號」，相同服務名稱會自動歸組；點服務名稱即可展開帳號。

第一次使用請到「鋒兄設定」建立或補齊 `trialpurchase`。可用「匯出 CSV／匯入 CSV」備份或批次更新；相同服務與帳號會覆蓋，其餘新增。刪除前會跳出確認，預設焦點在「取消」。完整欄位與操作見 [16_trial_purchase.md](./16_trial_purchase.md)。

---

## 重灌軟體

入口：**鋒兄管理 → 重灌**。整理 Windows／Mac 重灌時需要的軟體、網站與授權。付費序號在清單與表單都預設隱藏；若設了查看密碼，清單需先輸入密碼才顯示序號。訂閱制軟體可記下週期（年／月）與費用（台幣／美元／日圓／人民幣）。可用「匯出 CSV／匯入 CSV」備份或批次更新；相同服務名稱與系統會覆蓋，其餘新增。CSV 含序號與查看密碼，請妥善保管。

第一次使用請到「鋒兄設定」建立或補齊 `reinstall`。刪除前會跳出確認，預設焦點在「取消」。完整欄位與操作見 [17_reinstall.md](./17_reinstall.md)。
...
1. **一鍵匯出 CSV**：下載含所有 CSV 選单的 ZIP（不含媒體檔）。
2. **一鍵匯入 CSV**：選擇該 ZIP（或單一 `.csv`），相同紀錄會更新、其餘新增。
3. **一鍵匯出全部**：CSV 再加上圖片／影片／音樂／播客／文件／筆記 ZIP。
4. **一鍵匯入全部**：還原 CSV 與媒體檔；媒體重新上傳可能較久。
5. **匯出／匯入到 Google 雲端硬碱**：卡下方填入 Google Client ID 與 API Key後，可直接上傳到雲端硬碱「鍵兄備份」資料夹，或從雲端硬碱選檔匯入，不需要先下載到本機。
...
目前資料儲存在 Appwrite 雲端，也可到 **「鋒兄設定」→「選單備份／還原」** 一鍵匯出／匯入：

- **所有 CSV 選單（不含 ZIP）**：一次打包各選單 `.csv`（不含媒體檔）
- **所有選單（.csv + .zip）**：CSV 再加上圖片／影片／音樂／播客／文件／筆記的 ZIP

各模組頁面仍可單獨匯出 CSV 或 ZIP。
